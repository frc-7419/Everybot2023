...
public class Auton extends SequentialCommandGroup {
  public Auton(DriveBaseSubsystem driveBaseSubsystem, ArmSubsystem armSubsystem, IntakeSubsystem intakeSubsystem, SwerveDriveFieldCentric swerveDriveFieldCentric) {
    addCommands(
      ...
      new ArmSetpointPID(armSubsystem, ArmConstants.highConeSetpoint).alongWith(new RunIntake(intakeSubsystem, 0.115)).withTimeout(5),
      // new RunArm(armSubsystem,-0.05).alongWith(new RunIntake(intakeSubsystem, -0.5)).withTimeout(1),
      new ArmSetpointPID(armSubsystem, ArmConstants.armIdle).alongWith(new RunIntake(intakeSubsystem, -0.5)).withTimeout(1),
      new WaitCommand(3),
